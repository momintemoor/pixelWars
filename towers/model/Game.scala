...
class Game {
  ...
  def checkForBaseDamage(): Unit = {
    ...
    for(player <- players.values){
      if(player.location.x <= (level.base.x + 0.5 + playerSize) && player.location.y <= (level.base.y + 0.5 + playerSize) && player.location.y >= (level.base.y + 0.5 - playerSize) && player.location.x >= (level.base.x + 0.5 - playerSize)){
        baseHealth -= 1
  ...
  def checkForPlayerHits(): Unit = {
    ...
    for(proj <- projectiles){
      for(player <- players.values){
        var distance = player.location.distance2d(proj.location)
